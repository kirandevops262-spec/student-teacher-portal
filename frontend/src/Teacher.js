...
function Teacher() {
  ...
  return (
    <Box>
      <Helmet><title>Teachers • Student–Teacher Portal</title></Helmet>
      <Heading mb={6}>Store Teacher Details</Heading>
      <Box as="form" onSubmit={handleSubmit} mb={8} maxW="lg">
        <VStack spacing={4} align="stretch">
          <FormControl isRequired>
            <FormLabel>Name</FormLabel>
            <Input name="name" value={teacherData.name} onChange={handleChange} placeholder="Enter teacher name" />
          </FormControl>
          <FormControl isRequired>
            <FormLabel>Subject</FormLabel>
            <Input name="subject" value={teacherData.subject} onChange={handleChange} placeholder="Enter subject" />
          </FormControl>
          <FormControl isRequired>
            <FormLabel>Class</FormLabel>
            <Input name="class" value={teacherData.class} onChange={handleChange} placeholder="Enter class" />
          </FormControl>
          <HStack>
      ...
      {loading ? (
        ...
        <Box overflowX="auto">
          <Table size="sm" variant="simple">
            <Thead>
              <Tr><Th>ID</Th><Th>Name</Th><Th>Subject</Th><Th>Class</Th><Th textAlign="center">Actions</Th></Tr>
            </Thead>
            <Tbody>
              {data.map((d, i) => {
                const id = d.id || i;
